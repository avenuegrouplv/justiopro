import React from 'react';
import { Link } from 'react-router-dom';
import {
  FileText,
  Sparkles,
  ArrowLeft,
  Phone,
  Mail,
  FolderOpen,
  Search,
  Download,
  CheckCircle2,
  Clock,
  ArrowRight,
  HelpCircle,
  PhoneCall,
  Shield,
  FileCheck,
} from 'lucide-react';
import SEOHead from '../components/SEOHead';
import { COMPANY_INFO } from '../data/content';
import SectionNavButtons from '../components/SectionNavButtons';

interface DocumentSamplesPageProps {
  onOpenLightbox?: (src: string, alt: string, caption?: string) => void;
}

export default function DocumentSamplesPage({ onOpenLightbox }: DocumentSamplesPageProps) {
  return (
    <div id="document-samples-page" className="min-h-screen bg-[#CDD4DC] text-slate-900">
      <SEOHead
        title="Dokumentu paraugi"
        description="JustioPro juridisko dokumentu un līgumu paraugi: pirkuma, īres, pakalpojumu, aizdevuma, darba un citi civiltiesiskie līgumi un iesniegumi."
      />

      {/* Hero Header Banner */}
      <section className="relative overflow-hidden bg-gradient-to-b from-[#0B1F33] to-[#06121E] pt-20 sm:pt-24 pb-16 sm:pb-20 text-white border-b border-black">
        <div className="absolute inset-0 opacity-35">
          <img
            src="/profesionali-juridiskie-pakalpojumi.webp"
            alt="Dokumentu paraugu fons"
            className="h-full w-full object-cover"
          />
        </div>
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center md:text-left translate-y-[5mm]">
          {/* Top navigation & Contact Info bar */}
          <div className="mb-6 -mt-2 sm:-mt-3 flex flex-wrap items-center justify-between gap-3">
            <Link
              to="/"
              className="inline-flex items-center gap-2 rounded-xl border border-white/25 bg-white/10 px-4 py-2 text-xs sm:text-sm font-medium text-white shadow-xs backdrop-blur-xs transition-colors hover:bg-white/20 hover:border-[#C9A45C] hover:text-[#C9A45C]"
            >
              <ArrowLeft className="h-4 w-4" />
              <span>Uz Sākumu</span>
            </Link>

            <div className="flex flex-wrap items-center gap-3 translate-y-[-3mm]">
              <a
                href={`tel:${COMPANY_INFO.phone}`}
                className="inline-flex items-center gap-2.5 rounded-xl border border-white/35 bg-[#06121E]/90 px-4 py-2 sm:px-4.5 sm:py-2.5 text-xs sm:text-sm font-medium text-white shadow-lg backdrop-blur-md transition-all hover:bg-[#0B1F33] hover:border-[#C9A45C] hover:text-[#C9A45C] cursor-pointer"
                title="Zvanīt"
              >
                <Phone className="h-4 w-4 text-[#C9A45C]" />
                <span>{COMPANY_INFO.phoneDisplay}</span>
              </a>

              <a
                href={`mailto:${COMPANY_INFO.email}`}
                className="inline-flex items-center gap-2.5 rounded-xl border border-white/35 bg-[#06121E]/90 px-4 py-2 sm:px-4.5 sm:py-2.5 text-xs sm:text-sm font-medium text-white shadow-lg backdrop-blur-md transition-all hover:bg-[#0B1F33] hover:border-[#C9A45C] hover:text-[#C9A45C] cursor-pointer"
                title="Rakstīt e-pastu"
              >
                <Mail className="h-4 w-4 text-[#C9A45C]" />
                <span>{COMPANY_INFO.email}</span>
              </a>
            </div>
          </div>

          <div className="inline-flex items-center gap-2 rounded-full border border-[#C9A45C]/50 bg-[#06121E] px-4 py-1.5 text-xs font-medium uppercase tracking-widest text-[#C9A45C]">
            <Sparkles className="h-3.5 w-3.5" />
            <span>Līgumi & Juridiskie dokumenti</span>
          </div>
          <h1 className="mt-4 text-4xl sm:text-5xl font-light tracking-tight text-white leading-tight">
            Dokumentu <span className="font-semibold text-[#C9A45C]">paraugi</span>
          </h1>
          <p className="mt-3 max-w-2xl text-sm sm:text-base font-light text-slate-300">
            Pārbaudīti un juridiski korekti līgumu, vienošanos un pieteikumu paraugi Jūsu darījumiem un ikdienas tiesiskajām situācijām.
          </p>
        </div>
      </section>

      {/* Main Content Section */}
      <section className="relative overflow-hidden py-14 sm:py-18 bg-[#CDD4DC]">
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-10">
          
          {/* Welcome Info Card */}
          <div className="rounded-2xl border border-black/70 bg-white p-6 sm:p-8 shadow-xs">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-2.5 max-w-3xl">
                <div className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[#C9A45C]">
                  <FileCheck className="h-4 w-4 text-[#C9A45C]" />
                  <span>Juridiski precīzi dokumentu šabloni</span>
                </div>
                <h2 className="text-2xl sm:text-3xl font-medium text-[#0B1F33]">
                  Līgumu un juridisko dokumentu krātuve
                </h2>
                <p className="text-xs sm:text-sm font-light text-slate-700 leading-relaxed">
                  Šeit drīzumā būs pieejami strukturēti un praktiski izmantojami dokumentu paraugi dažādām dzīves un biznesa situācijām. Ja Jums nepieciešams individuāli izstrādāts līgums vai esoša dokumenta juridiskā analīze, mūsu juristi ir gatavi Jums palīdzēt nekavējoties.
                </p>
              </div>

              <div className="shrink-0">
                <Link
                  to="/kontakti"
                  className="inline-flex items-center gap-2 rounded-xl border border-black bg-[#0B1F33] px-5 py-3 text-xs sm:text-sm font-semibold uppercase tracking-wider text-white shadow-md transition-all hover:bg-[#122B45] hover:border-[#C9A45C] active:scale-95"
                >
                  <PhoneCall className="h-4 w-4 text-[#C9A45C]" />
                  <span>Pieteikt dokumentu izstrādi</span>
                </Link>
              </div>
            </div>
          </div>

          {/* Categories Placeholder Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="rounded-2xl border border-black/70 bg-white p-6 shadow-xs space-y-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-black bg-[#0B1F33] text-[#C9A45C]">
                <FileText className="h-5 w-5" />
              </div>
              <h3 className="text-lg font-semibold text-[#0B1F33]">Darījumu un sadarbības līgumi</h3>
              <p className="text-xs font-light text-slate-600 leading-relaxed">
                Pirkuma, piegādes, pakalpojumu, sadarbības un konfidencialitātes (NDA) līgumu paraugi.
              </p>
            </div>

            <div className="rounded-2xl border border-black/70 bg-white p-6 shadow-xs space-y-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-black bg-[#0B1F33] text-[#C9A45C]">
                <FolderOpen className="h-5 w-5" />
              </div>
              <h3 className="text-lg font-semibold text-[#0B1F33]">Nekustamā īpašuma dokumenti</h3>
              <p className="text-xs font-light text-slate-600 leading-relaxed">
                Dzīvojamo telpu īres, komerctelpu nomas, rokasnaudas un pieņemšanas-nodošanas akti.
              </p>
            </div>

            <div className="rounded-2xl border border-black/70 bg-white p-6 shadow-xs space-y-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-black bg-[#0B1F33] text-[#C9A45C]">
                <Shield className="h-5 w-5" />
              </div>
              <h3 className="text-lg font-semibold text-[#0B1F33]">Prasības un iesniegumi</h3>
              <p className="text-xs font-light text-slate-600 leading-relaxed">
                Pirmstiesas brīdinājumu, pretenziju un iesniegumu paraugi valsts un pašvaldību iestādēm.
              </p>
            </div>
          </div>

          {/* Section Navigation Buttons */}
          <SectionNavButtons className="mt-8" />
        </div>
      </section>
    </div>
  );
}
